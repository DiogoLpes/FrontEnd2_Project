...
import { authOptions } from "../lib/auth"; // ajusta o caminho conforme a tua estrutura
...
export default async function StatusPage({
  ...
  const userIdParsed = session?.user
    ? Number((session.user as any).id)
    : undefined;

  // Construir condições dinamicamente (evita undefined no Prisma)
    ...
    return (
      <div className="min-h-screen bg-[#05070a] text-white flex flex-col items-center justify-center gap-6 p-6 text-center">
        ...
        <div>
          ...
          <p className="text-slate-500 text-[10px] mt-2 uppercase font-black tracking-[0.2em]">
            Não encontramos registos para:{" "}
        ...
        <Link
          href="/agenda"
          className="bg-blue-600 px-10 py-4 rounded-2xl font-black uppercase italic text-xs hover:bg-white hover:text-black transition-all shadow-lg shadow-blue-600/20"
        >
          Nova Marcação
        </Link>
      </div>
