    ...
    <div className="flex min-h-screen bg-[#02040a] text-slate-200">
      ...
      <aside className="w-64 border-r border-white/5 bg-[#09090b] hidden md:flex flex-col sticky top-0 h-screen">
        <div className="p-6 flex items-center gap-3">
          ...
          <span className="text-lg font-bold tracking-tighter text-white">TSPNEUS <span className="text-blue-600 italic font-black">PRO</span></span>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-1.5 mt-4">
          {menuItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center gap-3 px-3 py-2.5 rounded-md text-slate-400 hover:text-white hover:bg-white/[0.03] transition-all text-sm font-medium border border-transparent hover:border-white/5"
            >
              {item.icon}
              {item.label}
            </Link>
        ...
        <div className="p-4 mt-auto">
          ...
          <button className="flex items-center gap-3 px-3 py-2.5 w-full text-slate-500 hover:text-red-500 transition-all font-medium text-sm">
            <LogOut size={18} />
            Terminar Sessão
          </button>
        </div>
